...
import { Calendar, User, Tag, CheckCircle, AlertCircle } from "lucide-react"
...
export function History({ patientId }: HistoryProps) {
  ...
  const handleViewDetails = async (actionItem: Action) => {
    ...
    try {
      const appointmentData = await appointment.getAppointmentsByActionId(actionItem.id)
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>A record of all your past appointments.</CardDescription>
      </CardHeader>
      <CardContent>
        {actions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-muted-foreground max-w-md mt-2">You don't have any past appointments in your history.</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {actions.map((action) => {
                const appointment = action.appointments && action.appointments[0]
