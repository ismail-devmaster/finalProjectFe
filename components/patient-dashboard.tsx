  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const actionsData = await action.getActionsByPatientId(patientIdData.patientId)
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <Button onClick={() => setIsNewActionModalOpen(true)}>New Action</Button>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {actions.map((action) => (
          <PatientCard key={action.id} action={action} />
        ))}
      </div>
      <NewActionModal
